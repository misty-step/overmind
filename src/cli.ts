#!/usr/bin/env npx tsx
/**
 * Overmind - Portfolio command center for indie hackers
 *
 ...
 *
 * Usage:
 *   overmind                    # All products, terminal output
 *   overmind --html             # Export to HTML and open
 *   overmind volume             # Filter to single product
 */
...
function loadProducts(): Product[] {
  const configPath = join(__dirname, "..", "config", "products.yaml");
...
function renderTerminal(metrics: ProductMetrics[]): string {
  ...
  let output = `
┌─────────────────────────────────────────────────────────────────────────────────┐
│ OVERMIND - Week of ${weekOf.padEnd(58)}│
├─────────────────────────────────────────────────────────────────────────────────┤
...
async function main() {
  ...
  if (htmlMode) {
    ...
    const outPath = join(homedir(), "overmind.html");
